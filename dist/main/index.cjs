...
require$$1$2.ipcMain.handle("electron-store", async (_evnet, methodSign, ...args) => {
  if (typeof store[methodSign] === "function") {
    return store[methodSign](...args);
  }
  return store[methodSign];
});
const APP_NAME = require$$1$2.app.name;
require$$1$2.app.getVersion();
...
async function createWindow() {
  win = new require$$1$2.BrowserWindow(__spreadValues({}, DEFAULT_WINDOW_OPTIONS));
  ...
  require$$1$2.ipcMain.on("get-path", (event, args) => {
    event.returnValue = require$$1$2.app.getPath(args);
  });
}
...
require$$1$2.app.on("activate", () => {
  const allWindows = require$$1$2.BrowserWindow.getAllWindows();
