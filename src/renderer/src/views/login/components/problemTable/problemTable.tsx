...
const defaultColumns: (ColumnType<UnArray<GetProblemsResp['data']['questions']>>)[] = [
  ...
  },
  {
    ...
    dataIndex: 'acRate',
    key: 'acRate',
    render: (acRate: number, record) => {
      return <>{`${(acRate * 100).toFixed(2)}%`}</>;
    }
  },
  {
    title: '难度',
    ...
    key: 'difficulty',
    render: (difficulty: Difficulty, record) => {
  ...
  },
  {
    ...
    render: (topicTags: UnArray<GetProblemsResp['data']['questions']>['topicTags'], record) => {
      return <>{topicTags?.map(e => <Tag>{e.nameTranslated || e.name}</Tag>)}</>;
    }
  },
];

function ProblemTable<RecordType extends UnArray<GetProblemsResp['data']['questions']>>(props: React.PropsWithChildren<ProblemTableProps<RecordType>>) {
  const { tableStatus: { isLoading = false, pagination={} }, tableConst: { dataSource = [], columns = defaultColumns } } = props;
  ...
  return (
    <>
      <Table dataSource={dataSource || []} loading={isLoading} columns={columns as ColumnsType<RecordType>} pagination={pagination}/>
    </>
  );
};
